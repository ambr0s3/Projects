 ...
 * Created by AS
 */
public class GrafoPonderado<T extends Comparable<T>> implements IGraph<T> {
    ...
    public class Edge implements Comparable<Edge> {
        ...
        public Edge(int destino, int weight) {
        ...
        public boolean equals(Object obj) {
            return destino.equals((Integer) obj);
        }
    ...
    @Override
    public int numEdges() throws Exception //Te devuelve el numero de ejes
    {
        int count = 0;
        for (int i = 0; i < vertices.size(); i++)
    ...
    @Override
    public boolean hasVertex(T v) throws Exception //Te dice si hay vertice
    {
        return getPosition(v) != -1;
    }

    @Override
    public boolean hasEdge(T v1, T v2) throws Exception //Busca si hay un eje en los vertices
    {
    ...
    @Override
    public void addVertex(T v) throws Exception //Agrega un nuevo vertice
    {
        if (!hasVertex(v)) {
            vertices.addLast(v);
    ...
    @Override
    public void addEdge(T lado1, T lado2, int weight) throws Exception //Agrega eje
    {
        int p1 = getPosition(lado1);
        int p2 = getPosition(lado2);

        if ((p1 != -1) && (p2 != -1) && (!hasEdge(lado1, lado2))) {
            IList<Edge> adj = adjList.get(p1);
            adj.addLast(new Edge(p2, weight));
            if (p1 != p2) {
                adj = adjList.get(p2);
                adj.addLast(new Edge(p1, weight));
            }
        } else
    ...
    @Override
    public void replaceVertex(T v1, T v2) throws Exception //Reemplaza los vertices
    {
    ...
    public int[] maxLongestPath(T src) throws Exception {
    ...
    public IList<T> maxLongestPath(T src, T dest) throws Exception {
        //Initialize single source
        int UNDEFINED = -1;
        int[] dist = new int[numVertices()];
        int[] prev = new int[numVertices()];
